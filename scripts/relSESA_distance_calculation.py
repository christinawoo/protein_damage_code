...
# Errors occuring in ChimeraX analysis of individual PDBs will get logged to this filepath.
# Path to output csv file (should include the desired filename ending in .csv)
CHIMERAX_ERRORS_CSV_FILEPATH = ""

# Errors occurring when a residue has available PDBs in the input file, but ChimeraX analysis fails on all of the given PDBs.
# When this happens, the script will analyze an Alphafold-predicted model of the relevant protein instead.
# Residues for which there are are no available PDBs in the input file will NOT be logged here and will default to Alphafold.
# Path to output csv file (should include the desired filename ending in .csv)
EXHAUSTED_PDBS_ERRORS_CSV_FILEPATH = ""

# Errors occurring in analysis of an Alphafold-predicted protein after a residue's PDBs have been exhausted.
# Path to output csv file (should include the desired filename ending in .csv)
ALPHAFOLD_ERRORS_CSV_FILEPATH = ""
    ...
    # Otherwise, attempt PDB analysis
        ...
        while len(data['pdb_data_obj'][i]) > 0:
            ...
            try:
                ...
                # Exit loop upon successful analysis (stops trying additional PDBs from the object list)
                break
            except Exception as e:
...
with open(CHIMERAX_ERRORS_CSV_FILEPATH, 'w') as csvfile:
...
with open(ALPHAFOLD_ERRORS_CSV_FILEPATH, 'w') as csvfile:
    fwriter = csv.writer(csvfile)

    for x in alphafold_errors:
        fwriter.writerow(x)

with open(EXHAUSTED_PDBS_ERRORS_CSV_FILEPATH, 'w') as csvfile:
    fwriter = csv.writer(csvfile)
