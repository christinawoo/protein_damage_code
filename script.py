...
def analyze(uniprot, is_experimental, pdb, chain, aa_position):
    ...
    # Should work for both pdb structures and alphafold? Assuming that alphafold fetched structures only have one chain though
...
for i in map(str, range(length)):
    ...
    try:
        ...
        runCmd('close all')
        chain_id = "/" + chain if isinstance(chain, str) else ""
        errors.append((uniprot_id, "Error in " + uniprot_id +
